...
class LRTabbarViewController: UITabBarController {
    ...
    func getChildController() -> [UIViewController] {
        let homeController = LRNavigationViewController(rootViewController: LRChatBootHomeViewController())
        let exploreController = LRNavigationViewController(rootViewController: LRChatBootExploreViewController())
        let historyController = LRNavigationViewController(rootViewController: LRChatBootHistoryViewController())
        ...
        exploreController.tabBarItem = UITabBarItem(title: nil, image: UIImage(named: "Tab_icon_explore_normal")?.withRenderingMode(UIImage.RenderingMode.alwaysOriginal), selectedImage: UIImage(named: "Tab_icon_explore_select")?.withRenderingMode(UIImage.RenderingMode.alwaysOriginal))
        exploreController.tabBarItem.imageInsets = UIEdgeInsets(top: 5, left: .zero, bottom: .zero, right: .zero)
        historyController.tabBarItem = UITabBarItem(title: nil, image: UIImage(named: "Tab_icon_history_normal")?.withRenderingMode(UIImage.RenderingMode.alwaysOriginal), selectedImage: UIImage(named: "Tab_icon_history_select")?.withRenderingMode(UIImage.RenderingMode.alwaysOriginal))
        historyController.tabBarItem.imageInsets = UIEdgeInsets(top: 5, left: .zero, bottom: .zero, right: .zero)
        
        return [homeController, exploreController, historyController]
    }
}
