...
class LRAppsFlyerStatistics: NSObject {
    ...
    public func trackEventStartTrial(product: SKProduct?) {
        ...
        AppsFlyerLib.shared().logEvent(trackEvent, withValues: [AFEventParamPrice: NSNumber.init(value: _p.price.doubleValue),AFEventParamCurrency:_p.priceLocale.currencyCode ?? ""])
    ...
    public func updateOriginalTransactionIdWithObject(_ productReceipt: ReceiptItem?, product: SKProduct?, receipt: String, productIdentifier: String) {
        ...
        let idfaParams: [String: String] = LRCleanDeviceTool.getIDFA()
        ...
        // 货币单位
        appFlyerParams[af_purchase_currency] = _p.priceLocale.currencyCode ?? ""
        // 设备ID地址
        appFlyerParams[af_device_id_address] = LRCleanDeviceTool.getOperatorsIP() ?? ""
        // 购买订单ID
